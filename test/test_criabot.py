...
def criabot_instance(criadex_credentials, mysql_credentials, redis_credentials):
    with (patch('criabot.criabot.RAGFlowSDK') as MockRAGFlowSDK,
        ...
        criabot._postgres_api = MockBotDatabaseAPI()
        criabot._redis_api = MockBotCacheAPI()
        yield criabot

@pytest.mark.asyncio
async def test_create_bot(criabot_instance):
    criabot_instance._postgres_api.bots.exists = AsyncMock(return_value=False)
    ...
    criabot_instance._postgres_api.bots.insert = AsyncMock(return_value=1)
    criabot_instance._postgres_api.bot_params.insert = AsyncMock(return_value=None)
    ...
    criabot_instance._postgres_api.bots.insert.assert_called_once()
    criabot_instance._postgres_api.bot_params.insert.assert_called_once()

@pytest.mark.asyncio
async def test_create_bot_that_exists(criabot_instance):
    criabot_instance._postgres_api.bots.exists = AsyncMock(return_value=True)
...
async def test_delete_bot(criabot_instance):
    criabot_instance._postgres_api.bots.retrieve_id = AsyncMock(return_value=1)
    ...
    criabot_instance._postgres_api.bot_params.delete = AsyncMock()
    criabot_instance._postgres_api.bots.delete = AsyncMock()

    await criabot_instance.delete(name="existing_bot")

    assert criabot_instance._criadex.manage.delete.call_count == 2
    criabot_instance._postgres_api.bot_params.delete.assert_called_once_with(bot_id=1)
    criabot_instance._postgres_api.bots.delete.assert_called_once_with(name="existing_bot")
...
async def test_about_bot(criabot_instance):
    ...
    criabot_instance._postgres_api.bots.retrieve = AsyncMock(return_value=bots_model)
    criabot_instance._postgres_api.bot_params.retrieve = AsyncMock(return_value=params_model)
